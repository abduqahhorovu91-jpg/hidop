...
def db_save_saved_videos(data):
    try:
        ...
        logger.warning(f"Error saving saved videos: {e}")

def db_save_users(data):
    try:
...
def db_save_video_catalog(data):
    try:
        with open(VIDEO_CATALOG_FILE, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
...
ESP_START_MESSAGE = "race.x299_299_1"
ESP_STOP_MESSAGE = "race.x299_299_2"
ESP_ACTIVE_MESSAGE = "esp activet"
ESP_REPLY_VARIANTS = ["qabul qilindi", "bog'landi", "ishlayapti"]
...
def build_video_caption(video_id: int, video: dict[str, object]) -> str:
...
def build_webapp_url(user_id: int | None = None) -> str:
    base_url = WEBAPP_URL.rstrip("/")
    if user_id is None:
        return base_url
    separator = "&" if "?" in base_url else "?"
    return f"{base_url}{separator}id={user_id}"


def get_saved_videos_for_owner(owner_id: int) -> list[dict[str, object]]:
    saved_videos = SAVED_VIDEOS.get(owner_id, [])
    if not isinstance(saved_videos, list):
        return []

    items: list[dict[str, object]] = []
    for saved_item in saved_videos:
        if not isinstance(saved_item, dict):
            continue
        raw_video_id = saved_item.get("video_id", saved_item.get("saved_id"))
...
def remove_saved_video_for_owner(owner_id: int, video_id: int) -> dict[str, object] | None:
    saved_videos = SAVED_VIDEOS.get(owner_id, [])
    if not isinstance(saved_videos, list):
        return None
    ...
    for video in saved_videos:
        if not isinstance(video, dict):
            continue
        saved_id = parse_int(video.get("saved_id"))
        catalog_video_id = parse_int(video.get("video_id"))
        if saved_id == video_id or catalog_video_id == video_id:
            removed_video = video
            continue
    ...
    SAVED_VIDEOS[owner_id] = updated_videos
    save_saved_videos()
...
async def resolve_telegram_file_info(file_id: str) -> dict[str, object]:
    ...
    if not file_url:
        raise RuntimeError("Telegram file URL topilmadi.")
...
class BackgroundWebServer:
    ...
    def start(self) -> None:
        ...
        self._thread.start()
        logger.info("Web server ishga tushdi: http://%s:%s", self.host, self.port)
        print(f"🌐 Web server ishga tushdi: http://localhost:{self.port}")
...
def receive_esp_message():
    ...
    interval_ms = parse_int(payload.get("intervalMs"))
    device_url = str(payload.get("deviceUrl", "") or "").strip()

    logger.info(
        "ESP xabar qabul qilindi: message=%s interval=%s device_url=%s",
        message or "<empty>",
        interval_ms,
        device_url or "<empty>",
    )
        ...
        return jsonify(
            {
                ...
                "accepted": True,
                "message": message,
                "reply": "esp active qabul qilindi",
            }
        )
...
def search_videos():
    ...
    try:
        results = get_videos_by_name(query)
        return jsonify({"success": True, "items": results})
    except Exception as exc:
        return jsonify({"success": False, "error": str(exc)})


@web_app.route("/api/saved-videos")
def get_saved_videos():
    raw_owner_id = request.args.get("owner_id") or request.args.get("user_id") or ""
...
def send_video_api():
    payload = request.get_json(silent=True) or {}
    target_user_id = parse_int(payload.get("target_user_id"))
    video_id = parse_int(payload.get("video_id"))

    if target_user_id is None or video_id is None:
        return jsonify({"ok": False, "error": "target_user_id va video_id kerak"}), 400
...
@web_app.route("/api/delete-saved-video", methods=["POST"])
def delete_saved_video_api():
    payload = request.get_json(silent=True) or {}
    owner_id = parse_int(payload.get("owner_id"))
    video_id = parse_int(payload.get("video_id"))

    if owner_id is None or video_id is None:
        return jsonify({"ok": False, "error": "owner_id va video_id kerak"}), 400

    removed_video = remove_saved_video_for_owner(owner_id, video_id)
    if not removed_video:
        return jsonify({"ok": False, "error": "Video topilmadi"}), 404

    video_name = str(removed_video.get("name", "")).strip()
    return jsonify(
        {
            "ok": True,
            "message": f"{video_name} saqlanganlardan olib tashlandi 🗑️." if video_name else "Video o'chirildi.",
        }
    )
...
def build_video_file_response(video_id: int):
    video = get_video_by_number(video_id)
    if not video:
        return jsonify({"success": False, "error": "Video not found"}), 404
